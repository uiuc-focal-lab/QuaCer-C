...
def get_args():
    ...
    parser.add_argument('--qa_llm', type=str, default='lmsys/vicuna-7b-v1.5-16k')
...
def query_vicuna_model(prompts: list[str], model, tokenizer, do_sample=True, top_k=10, 
    ...
    input_ids = tokenizer(input_ids, return_tensors="pt", padding=True).to(INPUT_DEVICE)
...
def experiment_pipeline(graph_algos, graph_text_edge, entity_aliases, source, relation_aliases, id2name, k=5, distractor_query=False, num_queries=5):
    ...
    with torch.no_grad():
        for num_iter in range(num_queries//BATCH_NUM):
            ...
            model_answers= query_vicuna_model(prompts, qa_model, tokenizer, temperature=1.0)
